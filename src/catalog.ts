...
const infra: Track[] = [
  ...
  row(
    ...
    "tar -czf dist/api-${BUILD_NUMBER}.tgz -C dist api && sha256sum dist/api-${BUILD_NUMBER}.tgz > dist/api-${BUILD_NUMBER}.tgz.sha256",
    "sha256sum -c dist/api-${BUILD_NUMBER}.tgz.sha256",
  ...
  row(
    ...
    "oc set image deployment/api api=registry.example.com/payments/api:nonroot-1.4.3",
  ...
  row(
    ...
    "kubectl annotate ingress api acme.cert-manager.io/http01-ingress-class=nginx --overwrite",
  ...
  row(
    ...
    "CLIENT_UUID=$(kcadm.sh get clients -r prod -q clientId=portal --fields id --format csv --noquotes) && kcadm.sh update clients/$CLIENT_UUID -r prod -s 'redirectUris=[\"https://portal.example.com/*\"]'",
  ...
  row(
    ...
    `curl -fsS --get "$PROM_URL/api/v1/query" --data-urlencode 'query=histogram_quantile(0.95, sum by (le) (rate(http_request_duration_seconds_bucket[5m])))' | grep -q '"status":"success"'`,
  ...
  row(
    ...
    "Server available",
    "otelcol validate --config /etc/otel/config.yaml && sudo systemctl restart otelcol",
  ...
  row(
    ...
    `curl -fsS -X PUT -H "Authorization: Bearer $CF_TOKEN" -H 'Content-Type: application/json' --data @ruleset.json "https://api.cloudflare.com/client/v4/zones/$ZONE/rulesets/$RULESET"`,
...
const data: Track[] = [
  ...
  row(
    ...
    "kubectl set env deployment/payments-consumer PARTITION_KEY_STRATEGY=customer-order",
  ...
  row(
    ...
    "dagster asset materialize -s monthly_revenue --partition 2026-07 | grep -q RUN_SUCCESS",
  ...
  row(
    ...
    `databricks jobs run-now --json '{"job_id":'"$JOB_ID"',"job_parameters":{"mode":"dedupe"}}'`,
  ...
  row(
    ...
    `psql -Atc "SELECT count(*) FROM pg_stat_activity WHERE wait_event_type='Lock'" | grep -qx 0`,
...
const analytics: Track[] = [
  ...
  row(
    ...
    "tabcmd get '/views/Sales/Overview.csv?:showVizHome=no' -f baseline.csv",
...
const testing: Track[] = [
  row(
    ...
    "git apply cypress-network-wait.patch && npx cypress run --spec cypress/e2e/checkout.cy.ts",
  ...
  row(
    ...
    "git apply stable-role-locator.patch && npx playwright test checkout --project=chromium",
  ...
  row(
    ...
    "find target/gatling -mindepth 2 -name index.html -print -quit | grep -q index.html",
    [
      "Injection profile",
      "Virtual users",
      "Service saturation",
